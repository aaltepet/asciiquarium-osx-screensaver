...
open class EntityFullWidth: BaseEntity {

    public override init(name: String, type: EntityType, shape: [String], position: Position3D) {
        super.init(name: name, type: type, shape: shape, position: position)
        isFullWidth = true
    }

    open override func moveEntity(deltaTime: TimeInterval) -> Position3D? {
    ...
    open func getShape(for width: Int) -> [String] {
    ...
    open override func getBounds() -> BoundingBox {
